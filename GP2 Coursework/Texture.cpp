...
void Texture::init(const std::string& fileName)
{
	...
	if (imageData == NULL)
	{
		std::cerr << "texture load failed" << fileName << std::endl; //Prints eror if the imageData is null
	}

	glGenTextures(1, &textureHandler); // generates texture names (num of texture names, speficies the location to store the names)
	...
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); // linear filtering for minification (When the texture is smaller than the area it is being mapped too)
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR); // linear filtering for magnifcation (When the texture is larger than the area it is being mapped too)
